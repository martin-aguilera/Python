from ursina import Ursina, window, Light, camera, Grid, scene, print_on_screen, invoke
...
class Game(Ursina):
    ...
    def check_game_over(self):
        ...
        if (
            0 < snake[-1][0] < self.MAP_SIZE
            and 0 < snake[-1][1] < self.MAP_SIZE
            and len(snake) == len(set(snake))
        ):
            return
        print_on_screen("GAME OVER", position=(-0.01, 0.003), scale=0.15, duration=5)
