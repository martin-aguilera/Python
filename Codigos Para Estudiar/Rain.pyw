...
def detectar_colision(jugador_pos, enemigo_pos):
    ...
    if (ex >= jx and ex < (jx + jugador_size)) or (
        jx >= ex and jx < (ex + enemigo_size)
    ):
        if (ey >= jy and ey < (jy + jugador_size)) or (
            jy >= ey and jy < (ey + enemigo_size)
        ):
            return True
    return False
...
while not game_over:
    ...
    pygame.draw.rect(
        ventana,
        color_celeste,
        (enemigo_pos[0], enemigo_pos[1], enemigo_size, enemigo_size),
    )
    pygame.draw.rect(
        ventana,
        color_celeste,
