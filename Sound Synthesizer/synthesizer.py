...
for i in range(len(noteslist)):
    ...
    color = np.array(
        [
            np.sin(i / 25 + 1.7) * 130 + 125,
            np.sin(i / 30 - 0.21) * 215 + 40,
            np.sin(i / 25 + 3.7) * 130 + 125,
        ]
    )
    ...
    for event in pg.event.get():
        ...
        if event.type == pg.KEYDOWN:
            ...
            if key in keymod:
                ...
                screen.blit(
                    font.render(notes[key][1], 0, (255, 255, 255)), notes[key][3]
                )
        if (
            event.type == pg.KEYUP
            and str(event.unicode) != ""
            and str(event.unicode) in keylist
        ):
